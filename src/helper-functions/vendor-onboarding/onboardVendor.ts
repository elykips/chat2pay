...
async function runTenantMigrations(databaseUrl: string) {
  const db = knex({
    ...
    migrations: {
      directory: `${process.cwd()}/src/migrations/tenants`
    }
  })
...
async function provisionDedicatedDb(dbName: string) {
  ...
  try {
    ...
    // const sql = format(` DO $do$ BEGIN IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = %L) THEN EXECUTE 'CREATE ROLE %I LOGIN PASSWORD %L'; END IF; END $do$; `, user, user, pass) 
    // await admin.raw(sql)

    // console.log('await admin.raw(sql)', sql);
